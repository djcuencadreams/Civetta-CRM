...
export const customers = pgTable("customers", {
  ...
  source: varchar("source", { length: 50 }).default(sourceEnum.INSTAGRAM),
...
// Define product categories table first with self-reference handled in relations
export const productCategories = pgTable("product_categories", {
  ...
  parentCategoryId: integer("parent_category_id"), // Self-reference defined in relations
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull()
});
...
// Define sales table with relation to orders
...
export const insertLeadSchema = z.object({
  ...
  status: z.enum(Object.values(leadStatusEnum) as [string, ...string[]]).default(leadStatusEnum.NEW),
  source: z.enum(Object.values(sourceEnum) as [string, ...string[]]).default(sourceEnum.INSTAGRAM),
...
// Create select schemas for all tables
